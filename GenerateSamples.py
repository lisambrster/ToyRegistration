...
def main():
    ...
    bTest3D = False
    if bTest3D:
        x,y = generate_sample_3D()
        print(x.shape,y.shape)
        tiff.imsave('raw1.tif',x)
        tiff.imsave('label1.tif',y)
        exit()
    else:
        x,dvf = generate_sample_jiggle()
        ...
        tiff.imsave('raw1.tif',x[:,:,0])
        tiff.imsave('raw2.tif',x[:,:,1])

        # translation - random +/-
        #x2,y2 = transform_sample(x,y, 40)
        # save label image y
        tiff.imsave('label1.tif',dvf[:,:,0])
        # save raw image x
        tiff.imsave('label2.tif',dvf[:,:,1])
        print(dvf.shape)
        tiff.imsave('dvf_x.tif', dvf[:, :, 3])
        tiff.imsave('dvf_y.tif', dvf[:, :, 2])
        # note vector field has y axis inverted (in FIJI flip label images to match)
        DrawVectorField.MakeVectorFieldImage(dvf[:,:,2], dvf[:,:,3],'jiggle_dvf.jpg')
